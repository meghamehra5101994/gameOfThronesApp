...
Begin
    ...
    data1:=  (SELECT array_to_json(array_agg(row)) FROM (
        (select attacker_king,count(attacker_king) as active_attacker_king
    ...
    data2:=  (SELECT array_to_json(array_agg(row)) FROM (
        (select defender_king,count(defender_king) as active_defender_king
    ...
    data3:=  (SELECT array_to_json(array_agg(row)) FROM (
        (select region,count(region) as active_region
   ...
   most_active := '[{ "most_active_attacker" : ' || coalesce(data1, '[]')||',
		   "most_active_defender" : ' || coalesce(data2, '[]')||', "most_active_region" : '|| coalesce(data3,'[]') ||'}]';

   attacker_outcomes := '[{ "win" : ' || coalesce(data4, '[]')||',
		   "loss" : ' || coalesce(data5, '[]')||'}]';
